    ...
    if character_choice == "1. Bring your own character":
        ...
        if st.session_state.selected_character_index is not None:
            ...
            with col1:
                try:
                    st.image(
                        PREDEFINED_CHARACTERS[selected_index],
                        caption=f"Selected Character {selected_index + 1}",
                        width=200,
                    )
                except:
            ...
            with col2:
                ...
                if st.button(
                    ...
                    st.session_state.flow_data["character_selection"] = (
                        selected_index + 1
                    )
...
# Step 3: Dialog Generation
    ...
    if dialog_choice == "1. Enter custom dialog":
        ...
        if st.button("Confirm Auto Generate", type="primary"):
            ...
            st.session_state.flow_data["dialog_content"] = (
                "[Auto-generated dialog will be created]"
            )
            st.session_state.current_step = "summary"
            st.session_state.flow_completed = True
            st.rerun()
...
# Step 4: Summary
    ...
    with col1:
        ...
        if st.session_state.flow_data.get("character_choice") == "custom":
            ...
            st.write(
                f"- Selection: Character {st.session_state.flow_data.get('character_selection')}"
            )
            st.write(f"- URL: {st.session_state.flow_data.get('character_url')}")
        ...
        st.write(
            f"- Source: {st.session_state.flow_data.get('product_image_source', 'Unknown')}"
        )
    ...
    with col2:
        ...
        if st.session_state.flow_data.get("character_url"):
            ...
            try:
                st.image(
                    st.session_state.flow_data["character_url"],
                    caption="Selected Character",
                    width=200,
                )
            except:
                st.write(
                    f"Character URL: {st.session_state.flow_data['character_url']}"
                )
        ...
        if st.session_state.flow_data.get("product_image_url"):
            ...
            try:
                st.image(
                    st.session_state.flow_data["product_image_url"],
                    caption="Product Image",
                    width=200,
                )
            except:
                st.write(
                    f"Product URL: {st.session_state.flow_data['product_image_url']}"
                )
